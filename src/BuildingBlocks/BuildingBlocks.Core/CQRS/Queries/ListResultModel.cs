...
public record ListResultModel<T>(List<T> Items, long TotalPages, int Page, int PageSize)
    where T : notnull
{
    ...
    {
        return ListResultModel<U>.Create(Items.Select(map).ToList(), TotalPages, Page, PageSize);
    }
}
