...
{
    public static IQueryable<T> Filter<T>(this IQueryable<T> collection, string filters)
    {
        if (!string.IsNullOrEmpty(filters)) return collection = collection.Where(filters);
        return collection;
    }

    public static IQueryable<T> Filter<T>(this IQueryable<T> collection, IList<string> filters)
    {
    ...
    public static IQueryable<T> Sort<T>(this IQueryable<T> collection, string sorts)
    {
        if (!string.IsNullOrEmpty(sorts)) return collection = collection.OrderBy(sorts);
        return collection;
    }

    public static IQueryable<T> Sort<T>(this IQueryable<T> collection, IList<string> sorts)
    {
    ...
    where T : notnull
    {
        ...
        return ListResultModel<T>.Create(data, totalPages, page, pageSize);
    }
    ...
    where TR : notnull
    {
        ...
        return ListResultModel<TR>.Create(data, totalPages, page, pageSize);
    }
    ...
    where TEntity : class
    {
        if (navigationPropertiesPath is null) return source;
