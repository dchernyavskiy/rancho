    ...
    )
    {
        if (builder.Configuration.GetValue<bool>("PostgresOptions:UseInMemory"))
        {
            builder.Services.AddDbContext<IdentityContext>(
                options => options.UseInMemoryDatabase("Rancho.Services.Identity")
            );
        ...
        // Problem with .net core identity - will override our default authentication scheme `JwtBearerDefaults.AuthenticationScheme` to unwanted `Rancho.Services.Identity.Application` in `AddIdentity()` method .net identity
