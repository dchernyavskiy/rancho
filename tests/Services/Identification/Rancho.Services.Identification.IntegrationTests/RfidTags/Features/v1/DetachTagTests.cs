...
public class DetachTagTests : IdentificationServiceIntegrationTestBase
{
    private readonly AnimalCreatedV1 _message;
    private readonly RfidTag _tag;

    public DetachTagTests(
    ...
    ) : base(sharedFixture, outputHelper)
    {
        ...
        var r = new AttachRfidTag() {RfidTagId = _tag.Id, AnimalId = _message.Id};
        sharedFixture.SendAsync(r).Wait();
    }

    [Fact]
    [CategoryTrait(TestCategory.Integration)]
    public async Task ShouldDetachTagFromAnimalRecordInDb()
    {
        var request = new DetachRfidTag() {RfidTagId = _tag.Id};
